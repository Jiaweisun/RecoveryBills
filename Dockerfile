...
# VERSION 2 - EDITION 1
# Author: chenyongbing
# Cretae Time : 2016-09-26 上午10:33
# Command format: Instruction [arguments / command] ..
# Base image to use, this must be set as the first line

#FROM <image>  or FROM <image>:<tag>
#FROM 192.168.17.230:5000/kssz/centos7-jdk
...
ENV KS_PROJECT_PORT 8081

ENV JAVA_OPTS -Dproject.datacenter.id=1
RUN mkdir -p /data/$KS_PROJECT_NAME/bin && mkdir -p /data/$KS_PROJECT_NAME/logs
copy kop-reconciliation-manager-server/target/*.jar /data/$KS_PROJECT_NAME/libs/

EXPOSE $KS_PROJECT_PORT
VOLUME /data/$KS_PROJECT_NAME/resources
VOLUME /data/$KS_PROJECT_NAME/logs
